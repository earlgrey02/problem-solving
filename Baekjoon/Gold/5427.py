...
def fires_bfs(fires: set[tuple[int, int]], visited: list[list[float]]):
    ...
    while queue:
        ...
        for i in range(4):
            ...
            if 0 <= next_v[0] < h and 0 <= next_v[1] < w and matrix[next_v[0]][next_v[1]] == '.' and visited[next_v[0]][next_v[1]] == inf:
...
def escape_bfs(v: tuple[int, int], visited: list[list[int]]) -> int:
    ...
    while queue:
        ...
        for i in range(4):
            ...
            if 0 <= next_v[0] < h and 0 <= next_v[1] < w:
...
for _ in range(t):
    ...
    escape_visited = [[-1 for _ in range(w)] for _ in range(h)]
    fire_visited = [[inf for _ in range(w)] for _ in range(h)]
    fires = set()
